...
A refactoring audit (2026) covered `Sources/` (excluding `Tests/`, which served as the regression gate) for oversized functions/files, duplication, and performance opportunities. Most findings were fixed directly; these six were deliberately deferred because each needs a real design change rather than a contained, mechanical fix. Re-verify against current source before acting on any of these — this list is a map of where to look, not a guarantee the code hasn't shifted since.

### `cons`/`conj` onto a list is O(n), not O(1)

`Expr.list` (`Expr.swift`) is `[Expr]`, a plain Swift array — there is no persistent singly-linked-list type. `cons` (`CoreSequenceCore.swift`, `coreCons`) implements prepend as `.seq([args[0]] + elements)`, which copies the entire existing array. Real Clojure's list is `clojure.lang.PersistentList`, a true immutable linked list where `cons` is O(1) via structural sharing — a list can be `cons`ed onto repeatedly without ever paying for the elements behind the new head. Fixing this properly means introducing an actual persistent-list representation (a boxed cons-cell type) and threading it through everywhere `Expr.list` is pattern-matched — a real data-structure change, not a local fix. [swift-collections](https://github.com/apple/swift-collections) doesn't have anything to reach for here: its persistent structures are `TreeSet`/`TreeDictionary` (CHAMP-trie-based, same family as Clojure's `PersistentHashMap`/`PersistentHashSet`), not a list, and its `Deque` is O(1) amortized at both ends but COW rather than structurally-shared, so it doesn't give real persistence either — a cons-cell type would need to be hand-rolled.
