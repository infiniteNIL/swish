...
### AST rewriting — one shared walk

Every AST rewriter (`expandAliasesInExpr`, `macroexpandAll`, `preExpandSyntaxQuote`,
`syntaxQuoteExpand`, `substituteMutableFields`) delegates its collection recursion to
**`Expr.mappingChildren(_:)`** (`Expr+Walk.swift`), which rebuilds `.vector`/`.map`/
`.set`/`.sortedMap`/`.sortedSet` preserving case, metadata, and comparator. **A new
rewriter should handle only the forms it genuinely treats specially and delegate the
rest there** — these five each hand-rolled the same arms before. `.list`/`.seq` are
deliberately *not* covered: every rewriter special-cases them differently (skip `quote`,
scope `let`/`fn` locals, expand a macro head, splice `~@`), so they stay the caller's
job. Sorted collections are transformed **positionally**, not re-sorted (these walks
rewrite code, where reader order should survive); set collisions **drop** rather than
throwing `duplicateSetElement` as `eval` does.

### Native argument validation

Native (`Expr.nativeFunction`) bodies unwrap and validate arguments through the shared
`require*` helpers in **`CoreArgs.swift`** (`requireString`/`requireInteger`/
`requireSymbol`/`requireMap`/`requireAgent`/… plus `requireArgCount(_:in:/atLeast:/
exactly:function:)`), not hand-written `guard case … else { throw … }`. Each takes an
`@autoclosure message` so a site interpolating the offending value pays the formatting
only when it throws. **Message text is per-call-site on purpose** — tests assert exact
wording, and the codebase's existing inconsistencies (`"args"` vs `"arguments"`, a
`", got N"` suffix on some arity guards) were preserved rather than unified.
...
- **`rem`/`quot` cover the whole numeric tower** — both route through `coerceNumericPair` (`CoreArithmeticBasic.swift`) rather than a hand-rolled pairwise switch, so mixed `bigInteger`×`double` and `bigInteger`×`bigDecimal` operands work (`(rem 10N 3.0)` => `1.0`, `(quot 10N 3.0M)` => `3M`); they previously threw "arguments must be integers". The shared prologue `remQuotOperands` normalizes `.float`→`.double` and rejects a non-finite dividend / NaN divisor, but an **infinite divisor stays per-function** — `rem` answers NaN, `quot` answers `0.0` through the ordinary float path, matching Clojure.
...
### `set!` — dynamic-var form in `evalSet`; the field form is desugared before `evalSet`

`set!` (`evalSet`, `Evaluator+Binding.swift`) handles the `(set! var-symbol value)` form — mutating the current thread-local `binding` of a dynamic var (e.g. `(set! *print-length* 50)` inside a `binding` scope). A faithful port of `Var.doSet`: the target must be thread-bound or it throws Clojure's "Can't change/establish root binding" (no separate `isDynamic` check needed — a non-dynamic var is never thread-bound). No validator/watch involvement (Swish's `Var` has no validator). The **mutable-`deftype`-field form** `(set! field value)` is now supported, but never reaches `evalSet`: it's desugared into a `deftype-mutable-field-set!` native call by the definition-time method-body transform (see Protocols), so `evalSet` stays dynamic-var-only and a `(set! *dynamic-var* …)` inside a method still routes here. Java field/static assignment forms are permanently out of scope.
...
`is`'s general (non-`thrown?`) case dispatches through a real `assert-expr` multimethod (`test.clj`), giving rich failure output (`(not (= 5 4))` with real runtime values, not `(not false)`). `is`'s bare `thrown?`/`p/thrown?` stays a hardcoded `cond` branch (not `assert-expr`). **`thrown?`/`thrown-with-msg?` now check the declared class** `c` — `(thrown? ExceptionInfo …)` fails unless an `ExceptionInfo` (or subtype) is thrown — via a lenient guard: fail only when `c` resolves to a Swish type and the caught value isn't `(instance? c …)`; **stay lenient** for the catch-all names (`Exception`/`Throwable`/`Error`) and for a class name Swish can't resolve (e.g. a JVM class like `ArithmeticException`, which a stringified native error can't be an instance of anyway) — those pass on any throw. See the `catch` entry below for the type-dispatch basis.
...
### `catch` dispatches on the thrown value's type

`catch` clauses are matched **in order, first match wins**, on the thrown value's *type*, reusing `instance?`'s exact test (`catchClauseMatches`, `Evaluator+TryCatch.swift`): resolve the clause's type symbol (eval → `dispatchTypeName`) and compare against the thrown value's type name (`Expr.description`) via `builtinAncestors`. So `(catch ExceptionInfo e …)`, `(catch SomeRecord e …)`, and `(catch String e …)` (catches a thrown string, or a stringified native error) all work. `Exception`/`Throwable`/`Error` are **catch-alls** — Swish has no throwable hierarchy, so these match any thrown value (as `Exception` always did). A clause type that doesn't resolve to a Swish type (a typo, or a JVM class like `ArithmeticException` that Swish has no type for) is a **non-match** — the throw propagates, matching pre-fix behavior. Consequence of that + `exprForError` stringifying native `EvaluatorError`s: native errors are **type-erased** to `string`, so only a catch-all or a literal `(catch String e)`/`(catch Object e)` catches them — there is no way to `(catch <specific-native-type> e)`. Backs the real `thrown?`/`thrown-with-msg?` class checks (see clojure.test entry).
...
### `hash` implements Clojure's deterministic `hasheq` (not Swift's `Hasher`)
...
- **`resolveQualifiedVar` splits before it probes the cache** — the `splitQualified` guard must stay *above* the `qualifiedVarCache` read. Every cache key contains a `/`, so an unqualified name can never hit; probing first made every unqualified global reference on the eval hot path pay a `Mutex` acquisition plus a `String` hash to learn nothing.

- **`starNsVar` (the cached `*ns*` Var) — and `outVar`/`errVar` — need no invalidation** — same immutability as `qualifiedVarCache`: each is interned once at init and its Var object is never replaced (only its value changes, via `in-ns`/`setCurrentNs` or `binding`). `currentNs()`/`setCurrentNs()`/`currentOut()`/`currentErr()` read/write them directly instead of resolving by name, which `print`/`prn`/warnings would otherwise pay on every write. `registerIO` hands the two IO vars over via `Evaluator.cacheIOVars`. (The *remaining* deferral here is unrelated: `*ns*` is a plain non-dynamic Var, so `in-ns` is not thread-local — a threading-correctness concern documented in `Evaluator+Namespaces.swift`, deferred to whenever real background execution lands, not a perf item.)
