...
### `currentNs()` does two locked dictionary lookups on every fall-through symbol resolution

`currentNs()` (`Evaluator+Namespaces.swift`) does `findNs("clojure.core")!.findVar(name: "*ns*")!` — a locked namespace-registry lookup then a locked var-table lookup — on every `eval` of a bare symbol not found locally or as a qualified var. Not trivially fixable: `*ns*` is a normal (non-dynamic) `Var`, so `in-ns` mutates one shared root value rather than a thread-local binding; making it genuinely dynamic/thread-local is deferred to whenever real background execution lands.
