(ns clojure.core-test.bigint
  (:require [clojure.test :as t :refer [are deftest is]]
            [clojure.core-test.number-range :as r]
            [clojure.core-test.portability #?(:cljs :refer-macros :default :refer) [when-var-exists]]))

;; Swish-specific overlay for bigint.cljc from the Jank Clojure Test Suite.
;; The upstream :default branch checks (instance? clojure.lang.BigInt ...), a
;; JVM class Swish has no equivalent of. Swish's own bigint? predicate checks
;; exactly the same thing (arbitrary-precision integer) via its own type tag.

(when-var-exists bigint
  (deftest test-bigint
    (are [expected x] (= expected (bigint x))
      1N  1
      0N  0
      -1N -1
      #?@(:cljr [ r/max-double]
          :lpy [ r/max-double]
          :jank [ r/max-double]
          :cljs []
          :default [ r/max-double])
      1N  1.0
      0N  0.0
      -1N -1.0
      1N  "1"
      -1N "-1"
      #?@(:cljs []
          :default
          [1N  12/12
           0N  0/12
           -1N -12/12]))

    ;; Python VMs integer types are arbitrary precision and have no min or max, so
    ;; promotion is not relevant.
    #?@(:lpy []
        ;; Validate that we correctly promote from int to bigint with `inc'` and `dec'`.
        :default
        [(is (= 9223372036854775808N (inc (bigint r/max-int)) (inc' r/max-int)))
         (is (= -9223372036854775809N (dec (bigint r/min-int)) (dec' r/min-int)))])

    #?@(:jank []
        :cljs []
        :swish
        [(is (bigint? (bigint 0)))
         (is (bigint? (bigint 0.0)))
         (is (bigint? (inc' r/max-int)))
         (is (bigint? (dec' r/min-int)))]
        :default
        [(is (instance? #?(:lpy python/int :default clojure.lang.BigInt) (bigint 0)))
         (is (instance? #?(:lpy python/int :default clojure.lang.BigInt) (bigint 0.0)))
         (is (instance? #?(:lpy python/int :default clojure.lang.BigInt) (inc' r/max-int)))
         (is (instance? #?(:lpy python/int :default clojure.lang.BigInt) (dec' r/min-int)))])))
