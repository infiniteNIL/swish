...
;; Swish-specific overlay for short.cljc from the Jank Clojure Test Suite.
;; The upstream :default branch's instance? check uses a nested reader
;; conditional with only :cljr/:clj branches and no :default of its own
;; (`#?(:cljr System.Int16 :clj java.lang.Short)`). Swish's reader only ever
;; matches :swish or :default (never :cljr/:clj), and an unmatched
;; non-splicing `#?(...)` with no fallback contributes nothing at all —
;; which here silently drops the second argument to `instance?`, turning
;; it into a 1-arg call and throwing an arity error. This mirrors real
;; Clojure's own comment just above it: there is no platform-independent
;; predicate for "is this boxed as a short," and Swish has no boxed-Short
;; equivalent to check `instance?` against either (per this project's
;; standing rule to never fake JVM/clojure.lang types for ported tests —
;; see CLAUDE.md). So this branch is skipped for :swish, matching the
;; precedent already set by :lpy/:phel/:cljs immediately above it.
