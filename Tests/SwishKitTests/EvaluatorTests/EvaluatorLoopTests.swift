...
struct EvaluatorLoopTests {
    ...
    func loopRecurZeroBindings() throws {
        #expect(try eval("(loop [] 99)") == .integer(99))
    }
    ...
    func fnRecurCountDown() throws {
        _ = try eval("(defn count-down [n] (if (= n 0) :done (recur (- n 1))))")
        #expect(try eval("(count-down 5)") == .keyword("done"))
    }

    @Test("recur in fn accumulates result")
    func fnRecurFactorial() throws {
        _ = try eval("(defn fact [n acc] (if (= n 0) acc (recur (- n 1) (* n acc))))")
        #expect(try eval("(fact 5 1)") == .integer(120))
    }

    @Test("recur in fn with multiple params")
    func fnRecurMultiParam() throws {
        _ = try eval("(defn sum-to [n acc] (if (= n 0) acc (recur (- n 1) (+ acc n))))")
        #expect(try eval("(sum-to 10 0)") == .integer(55))
    }

    @Test("recur does not blow the stack for large iterations")
    func fnRecurLargeIteration() throws {
        _ = try eval("(defn loop-n [n] (if (= n 0) :done (recur (- n 1))))")
        #expect(try eval("(loop-n 10000)") == .keyword("done"))
    }
    ...
    func recurNotTailInFunctionArg() throws {
        #expect(throws: EvaluatorError.recurNotInTailPosition) {
            try eval("(defn fact [n] (* n (recur (- n 1))))")
        }
    }
