...
(defmacro cond->>
  ...
  is true.  Note that, unlike cond branching, cond->> threading does not
...
(defn partial
  ...
  returns a fn that takes a variable number of additional args and
  calls f with args + additional args."
  {:added "1.0"
   :static true}
  ([f] f)
  ([f arg1]
...
(defn trampoline
  ...
  returns that non-fn value. Note that if you want to return a
  function as a final value, you must wrap it in some data structure
...
(defmacro case
  ...
  Unlike cond and condp, case does not consider its clauses
  sequentially — in real Clojure this is a genuine constant-time
  dispatch (a JVM bytecode jump table); Swish has no bytecode compiler,
  so this port dispatches via a linear chain of equality checks
  instead, which is semantically identical but O(n) rather than O(1).
  All manner of constant expressions are acceptable in case, including
  numbers, strings, symbols, keywords, and (Clojure) composites
  thereof. Note that since lists are used to group multiple constants
  that map to the same expression, a vector can be used to match a
  list if needed. The test-constants need not be all of the same type."
  {:added "1.2"}
  [e & clauses]
  (let [ge (gensym)
