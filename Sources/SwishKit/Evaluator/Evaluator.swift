...
public class Evaluator {
    ...
    private func evalNs(_ elements: [Expr]) throws -> Expr {
        ...
        for directive in elements.dropFirst(2) {
            guard case .list(let parts) = directive,
                  !parts.isEmpty,
            ...
            case .vector(let parts):
                ...
                while i + 1 < parts.count {
                    ...
                    case "refer":
                        ...
                        case .vector(let syms):
                            let names = syms.compactMap { if case .symbol(let s) = $0 { s } else { nil } }
                            for symName in names {
                                guard let v = loadedNs.findVar(name: symName) else {
