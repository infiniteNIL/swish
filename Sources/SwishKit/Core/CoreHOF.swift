...
func registerHOF(into evaluator: Evaluator) {
    ...
    evaluator.register(name: "filter", arity: .fixed(2),
        doc: "Returns a lazy sequence of the items in coll for which (pred item) returns logical true. pred must be free of side-effects. Returns a transducer when no collection is provided.",
