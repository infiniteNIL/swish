...
func registerHOF(into evaluator: Evaluator) {
    evaluator.register(name: "apply",  arity: .atLeastOne) { [evaluator] args in try coreApply(evaluator, args) }
    evaluator.register(name: "map",    arity: .atLeastOne) { [evaluator] args in try coreMap(evaluator, args) }
    evaluator.register(name: "filter", arity: .fixed(2))   { [evaluator] args in try coreFilter(evaluator, args) }
    evaluator.register(name: "reduce", arity: .atLeastOne) { [evaluator] args in try coreReduce(evaluator, args) }
}
...
private func coreApply(_ evaluator: Evaluator, _ args: [Expr]) throws -> Expr {
    ...
    let tail: [Expr]
    switch lastArg {
    case .list(let elems, _):   tail = elems
    case .vector(let elems, _): tail = elems
    case .nil:                  tail = []
    default:
