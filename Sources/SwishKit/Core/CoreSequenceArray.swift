...
func registerSequenceArray(into evaluator: Evaluator) {
    evaluator.register(name: "aset", arity: .fixed(3),
        ...
        let sa = try requireArray(args[0], function: "aset", message: "first argument must be an array")
    ...
    evaluator.register(name: "aget", arity: .fixed(2),
        ...
        let sa = try requireArray(args[0], function: "aget", message: "first argument must be an array")
        guard case .integer(let idx) = args[1], idx >= 0, idx < sa.elements.count else {
    ...
    evaluator.register(name: "alength", arity: .fixed(1),
        ...
        let sa = try requireArray(args[0], function: "alength", message: "argument must be an array")
    ...
    evaluator.register(name: "aclone", arity: .fixed(1),
        ...
        arglists: [["array"]]) { args in
        let sa = try requireArray(args[0], function: "aclone", message: "argument must be an array")
